...
def _one_completion(client: Groq, messages: List[Dict[str, str]], temperature: float, model: str) -> str:
...
def generate_initial_answer(client: Groq, prompt: str, model: str) -> str:
    ...
    with cf.ThreadPoolExecutor(max_workers=3) as ex:
        futures = [
            ex.submit(_one_completion, client,
                     [{"role": "user", "content": prompt}], 0.9, model)
            for _ in range(3)
        ]
    ...
    return _one_completion(client, [{"role": "user", "content": synthesis_prompt}], 0.2, model)


def critique_answer(client: Groq, prompt: str, answer: str, model: str) -> str:
    ...
    return _one_completion(client, [{"role": "user", "content": critique_prompt}], 0.3, model)


def revise_answer(client: Groq, prompt: str, original_answer: str, critiques: str, model: str) -> str:
    ...
    return _one_completion(client, [{"role": "user", "content": revision_prompt}], 0.2, model)


def critique_improvement_loop(prompt: str, max_iterations: int = 2, groq_api_key: str | None = None, model: str = "llama-3.3-70b-versatile") -> Dict[str, Any]:
    ...
    with st.spinner("Generating initial answer..."):
        initial_answer = generate_initial_answer(client, prompt, model)
    ...
    for iteration in range(max_iterations):
        with st.spinner(f"Critiquing iteration {iteration + 1}..."):
            critiques = critique_answer(client, prompt, current_answer, model)

        with st.spinner(f"Revising iteration {iteration + 1}..."):
            revised_answer = revise_answer(client, prompt, current_answer, critiques, model)
...
st.markdown(
    "Generate high-quality answers through iterative critique and improvement using GPT-OSS (or other models)."
)
...
with st.sidebar:
    ...
    selected_model = st.selectbox(
        ...
        AVAILABLE_MODELS,
        index=0,
        help="Select the model to use for generation and critique."
    )
